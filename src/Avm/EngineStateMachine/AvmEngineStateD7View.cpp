...
VOID CAvmEngineStateD7View::Enter() const
{
    ALOGI("Enter state : %s !\n", GetStateName().c_str());
}

VOID CAvmEngineStateD7View::Exit() const
{
    ALOGI("Exit state : %s !\n", GetStateName().c_str());
}

BOOLEAN CAvmEngineStateD7View::ProcessMessage(UInt32 uiType, UInt32 uiMessageID, const string& pData) const
{
	ALOGI("CAvmEngineStateD7View::ProcessMessage : start!\n");
	ALOGI("CAvmEngineStateD7View::ProcessMessage : uiType is %d, uiMessageID is %d\n", (int)uiType,(int)uiMessageID);
	ALOGI("CAvmEngineStateD7View::ProcessMessage : pData is %s\n",pData.c_str()); 
	...
	if (csm != nullptr)
	{
		ALOGI("CAvmEngineStateD7View::ProcessMessage : Transition To TopView!\n");
...
#endif
	ALOGI("CAvmEngineStateD7View::ProcessMessage : end ret = %d\n",(int)ret);
	return ret;
}

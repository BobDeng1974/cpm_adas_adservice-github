...
VOID CAvmEngineStateRearWideAngleView::Enter() const
{
    ALOGI("Enter state : %s !\n", GetStateName().c_str());
}

VOID CAvmEngineStateRearWideAngleView::Exit() const
{
    ALOGI("Exit state : %s !\n", GetStateName().c_str());
}

BOOLEAN CAvmEngineStateRearWideAngleView::ProcessMessage(UInt32 uiType, UInt32 uiMessageID, const string& pData) const
{
	ALOGI("CAvmEngineStateRearWideAngleView::ProcessMessage : start!\n");
	ALOGI("CAvmEngineStateRearWideAngleView::ProcessMessage : uiType is %d, uiMessageID is %d\n", (int)uiType,(int)uiMessageID);
	ALOGI("CAvmEngineStateRearWideAngleView::ProcessMessage : pData is %s\n",pData.c_str()); 
	...
	if (csm != nullptr)
	{
		ALOGI("CAvmEngineStateRearWideAngleView::ProcessMessage : Transition To TopView!\n");
...
#endif
	ALOGI("CAvmEngineStateRearWideAngleView::ProcessMessage : end ret = %d\n",(int)ret);
	return ret;
}

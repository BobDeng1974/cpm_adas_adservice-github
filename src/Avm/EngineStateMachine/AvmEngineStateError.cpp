...
CAvmEngineStateError::~CAvmEngineStateError()
{}

VOID CAvmEngineStateError::Enter() const
{
    ALOGI("Enter state : %s !\n", GetStateName().c_str());
}

VOID CAvmEngineStateError::Exit() const
{
    ALOGI("Exit state : %s !\n", GetStateName().c_str());
}

BOOLEAN CAvmEngineStateError::ProcessMessage(UInt32 uiType, UInt32 uiMessageID, const string& pData) const
{
	ALOGI("CAvmEngineStateError::ProcessMessage : start!\n");
	ALOGI("CAvmEngineStateError::ProcessMessage : uiType is %d, uiMessageID is %d\n", (int)uiType,(int)uiMessageID);
	ALOGI("CAvmEngineStateError::ProcessMessage : pData is %s\n",pData.c_str()); 
	...
	if (csm != nullptr)
	{
		ALOGI("CAvmEngineStateError::ProcessMessage : Transition To TopView!\n");
...
#endif
	ALOGI("CAvmEngineStateError::ProcessMessage : end ret = %d\n",(int)ret);
	return ret;
}

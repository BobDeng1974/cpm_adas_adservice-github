...
VOID CAvmEngineStateOff::Enter() const
{
    ALOGI("Enter state : %s !\n", GetStateName().c_str());
}

VOID CAvmEngineStateOff::Exit() const
{
    ALOGI("Exit state : %s !\n", GetStateName().c_str());
}

BOOLEAN CAvmEngineStateOff::ProcessMessage(UInt32 uiType, UInt32 uiMessageID, const string& pData) const
{
    ALOGI("CAvmEngineStateOff::ProcessMessage : start!\n");
	ALOGI("CAvmEngineStateOff::ProcessMessage : uiType is %d, uiMessageID is %d\n", (int)uiType,(int)uiMessageID);
	ALOGI("CAvmEngineStateOff::ProcessMessage : pData is %s\n",pData.c_str());	
	...
	if (csm != nullptr)
	{
	    ALOGI("CAvmEngineStateOff::ProcessMessage : Transition To TopView!\n");
...
#endif
	ALOGI("CAvmEngineStateOff::ProcessMessage : end ret = %d\n",(int)ret);
	return ret;
}

...
VOID CAvmEngineStateTopView::Enter() const
{
    ALOGI("Enter state : %s !\n", GetStateName().c_str());
}

VOID CAvmEngineStateTopView::Exit() const
{
    ALOGI("Exit state : %s !\n", GetStateName().c_str());
}

BOOLEAN CAvmEngineStateTopView::ProcessMessage(UInt32 uiType, UInt32 uiMessageID, const string& pData) const
{
	ALOGI("CAvmEngineStateTopView::ProcessMessage : start!\n");
	ALOGI("CAvmEngineStateTopView::ProcessMessage : uiType is %d, uiMessageID is %d\n", (int)uiType,(int)uiMessageID);
	ALOGI("CAvmEngineStateTopView::ProcessMessage : pData is %s\n",pData.c_str()); 
	...
	if (csm != nullptr)
	{
		ALOGI("CAvmEngineStateTopView::ProcessMessage : Transition To TopView!\n");
...
#endif
	ALOGI("CAvmEngineStateTopView::ProcessMessage : end ret = %d\n",(int)ret);
	return ret;
}
